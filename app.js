...
const STORAGE_KEY="celam-data-v6";
...
function renderReminders(){
  ...
  c.querySelectorAll("[data-done]").forEach(b=>b.onclick=()=>{
    ...
    data.reminders[real].done=!data.reminders[real].done;save();render();
  });
  c.querySelectorAll("[data-delete]").forEach(b=>b.onclick=()=>{
    const r=rs[Number(b.dataset.delete)];
    data.reminders.splice(data.reminders.indexOf(r),1);save();render();
  });
}
...
$("#reminderForm")?.addEventListener("submit",e=>{
  ...
  save();$("#reminderDialog").close();render();
});
